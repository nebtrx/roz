...
namespace Roz.Data.EntityFramework.Scope
{
    ...
    /// 
    /// </summary>
    public class DbContextCollection : IDataContextCollection
    {
        ...
        public DbContextCollection(bool readOnly = false, IsolationLevel? isolationLevel = null, IDataContextFactory dbContextFactory = null)
        {
            _disposed = false;
            _completed = false;
        ...
        {
            if (!requestedType.IsClass || !requestedType.IsSubclassOf(typeof(DbContext)))
            ...
            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                ...
                DbContext dbContext = (DbContext)(_dbContextFactory != null
                    ? _dbContextFactory.CreateDataContext(requestedType)
        ...
        public TDataContext Get<TDataContext>() where TDataContext : class
        {
            if (!typeof(DbContext).IsAssignableFrom(typeof(TDataContext)))
                throw new InvalidOperationException("TDataContext must be derivable from DbContext");
            ...
            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                // First time we've been asked for this particular DbContext type.
                // Create one, cache it and start its database transaction if needed.
                DbContext dbContext = (DbContext) Convert.ChangeType((_dbContextFactory != null
                    ? _dbContextFactory.CreateDataContext<TDataContext>()
                    :  Activator.CreateInstance<TDataContext>()), typeof(DbContext));
            ...
            return _initializedDbContexts[requestedType] as TDataContext;
        }
